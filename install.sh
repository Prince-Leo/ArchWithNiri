...
# ==============================================================================
# Leo Arch Setup - Main Installer (v1.1))
# ==============================================================================
...
clean_intermediate_snapshots() {
  local config_name="$1"
  local start_marker="Before Leo Setup"
...
if [ -d "/root/ArchWithNiri" ]; then
  log "Removing installer from /root..."
  cd /
  rm -rfv /root/ArchWithNiri
else
  log "Repo cleanup skipped (not in /root/ArchWithNiri)."
...
log "Archiving log..."
if [ -f "/tmp/Leo_install_user" ]; then
  FINAL_USER=$(cat /tmp/Leo_install_user)
else
...
if [ -n "$FINAL_USER" ]; then
  ...
  mkdir -p "$FINAL_DOCS"
  cp "$TEMP_LOG_FILE" "$FINAL_DOCS/log-ArchWithNiri.txt"
  chown -R "$FINAL_USER:$FINAL_USER" "$FINAL_DOCS"
  echo -e "   ${H_BLUE}●${NC} Log Saved     : ${BOLD}$FINAL_DOCS/log-ArchWithNiri.txt${NC}"
fi
