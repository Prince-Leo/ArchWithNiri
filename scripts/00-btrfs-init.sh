...
# Snapshot Root
if snapper list-configs | grep -q "root "; then
  if snapper -c root list --columns description | grep -q "Before Leo Setup"; then
    log "Snapshot already created."
  else
    log "Creating Root snapshot..."
    if exe snapper -c root create --description "Before Leo Setup"; then
...
# Snapshot Home
if snapper list-configs | grep -q "home "; then
  if snapper -c home list --columns description | grep -q "Before Leo Setup"; then
    log "Snapshot already created."
  else
    log "Creating Home snapshot..."
    if exe snapper -c home create --description "Before Leo Setup"; then
