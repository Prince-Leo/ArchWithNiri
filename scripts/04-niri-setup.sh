...
SUDO_TEMP_FILE="/etc/sudoers.d/99_Leo_installer_temp"
...
if [ -d "$PARENT_DIR/dotfiles" ]; then
  ...
  if [ "$TARGET_USER" != "Leo" ]; then
    ...
    if [ -f "$EXCLUDE_FILE" ]; then
      ...
      while IFS= read -r item; do
        ...
        [ -n "$item" ] && [[ ! "$item" =~ ^# ]] && rm -rf "$PARENT_DIR/dotfiles/.config/$item"
      done <"$EXCLUDE_FILE"
    fi
  fi
  ...
  if [ "$TARGET_USER" != "Leo" ]; then
    ...
    WAYBARSTYLE_FILE="$HOME_DIR/.config/waybar/style.css"
    WAYBARSTYLE2_FILE="$HOME_DIR/.config/waybar-niri-Win11Like/style.css"

    # 如果文件存在，则执行替换操作
    if [ -f "$BOOKMARKS_FILE" ]; then
      # 使用 sed 将文件中的 "Leo" 全部替换为当前目标用户名
      # 使用 as_user 确保文件权限不会变成 root
      as_user sed -i "s/Leo/$TARGET_USER/g" "$BOOKMARKS_FILE"
      log "Updated GTK bookmarks path from 'Leo' to '$TARGET_USER'."
    fi
    if [ -f "$WAYBARSTYLE_FILE" ]; then
      as_user sed -i "s/Leo/$TARGET_USER/g" "$WAYBARSTYLE_FILE"
      log "Updated waybar style path from 'Leo' to '$TARGET_USER'."
    fi
    if [ -f "$WAYBARSTYLE2_FILE" ]; then
      as_user sed -i "s/Leo/$TARGET_USER/g" "$WAYBARSTYLE2_FILE"
      log "Updated waybar win11-style path from 'Leo' to '$TARGET_USER'."
    fi
    # --- 修改结束 ---
  fi
